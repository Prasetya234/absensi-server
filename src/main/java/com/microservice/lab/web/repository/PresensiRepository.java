...
public interface PresensiRepository extends JpaRepository<Presensi, String> {
    ...
    @Query(value = "SELECT * FROM presensi p JOIN user u ON p.user_id = u.id WHERE u.first_name LIKE CONCAT('%', ?1, '%') AND p.is_late = ?2 ORDER BY p.create_at DESC", nativeQuery = true)
